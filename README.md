...
- A **single jar** for the whole **1.21.3 to 26.2** range.
...
A moment later JEI reloads by itself, silently, with your server's recipes, and that reload is this plugin doing its job. Because that warning is confusing once it is no longer true, the plugin follows it with one line of its own:
...
- A **Paper**, **Purpur** or **Folia** server on Minecraft **1.21.3 to 26.2**.
...
| Value | Effect |
| --- | --- |
| `safe` *(default)* | No recipe payload, but still the full recipe book. **REI works normally for them.** JEI reads only the payload, so it shows no server recipes, and those players get one chat line explaining why. Nobody is disconnected. |
